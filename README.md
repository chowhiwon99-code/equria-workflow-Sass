...
외부 SaaS 구독 대신 직접 구축해 회사별로 커스터마이징하는 사내 SaaS. (제품 브랜드 = **Complow(컴플로우)** 확정·도메인 `complow.kr` — 코드의 `EQURIA`는 첫 사내 고객 흔적이자 내부 식별자로 의도적 유지)
...
4. 깊은 내용: `WORKLOG.md`(작업 로그) · `PRODUCTIZATION.md`(B2B 로드맵) · `AGENTS-MCP-STRATEGY.md` · `STUDY.md`(비개발자용 학습 코스)
5. 이미 구현 완료·과거 기록: `docs/archive/`(테넌트 격리·채팅허브·Google MCP 설계, 참고용만)
