...
위조 웹훅으로 유료 고객 강등 가능) 포함 신규 9건 발견, 아직 미수정(다음 세션 우선순위). ② 나이스페이가
"서비스 제공기간 3개월 초과 상품 판매불가"로 연간 결제 삭제 요구 → 대표 결정(삭제) → 즉시 반영.
...
### 아직 안 한 것 — 신규 리뷰 발견 9건 (다음 세션 우선순위)
1. 🔴 **결제 웹훅 위조 취약점**(CRITICAL) — `api/billing/nicepay/webhook/route.ts` 승인·취소 분기 둘 다
   `tid`가 실제로 그 `order_id` 소유인지 미검증. 자기 자신의 무관한 취소거래 tid로 남의 활성 구독을 강등
   시킬 수 있음. 갱신 order_id(`RNW-워크스페이스8자리-날짜`)가 완전 결정적이라 예측 가능.
2. SSRF(MCP 서버 등록, `lib/mcp/connect.ts`가 `safeFetch` 미사용) + IDOR(`api/finance/tax-invoice`가
   `workspace_id` 필터 없이 RLS만 의존 — 멀티워크스페이스 사용자가 타사 재무데이터로 세금계산서 발행 가능).
3. 어시스턴트·워크플로우실행이 멀티스텝 도구호출로 끝나면 응답 소실(에이전트채팅에서 이미 겪고 고친
   버그와 동일 클래스, 이번엔 다른 두 라우트에 적용 안 됨) — `api/assistant/route.ts`·`workflows/[id]/run`.
...
### 다음 세션 착수 후보
A. 위 발견 1(웹훅 위조) 수정 — 실 결제 무결성 문제라 최우선. B. 발견 2(SSRF·IDOR) 수정.
C. 발견 3(응답소실 버그) 수정 — 이미 검증된 fix 패턴 있어 빠름. D. 담당자 회신 메일 발송(대표 액션,
초안 있음 — 재심사 여부·2차인증 답변·바로오픈 문구 질문 3개 포함) 및 회신 확인 후 카드사 심사 접수 확인.
