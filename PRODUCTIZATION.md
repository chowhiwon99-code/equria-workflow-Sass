...
>
> ⚠️ **현행화(2026-08-18)**: 이 문서는 **로드맵·리스크 레지스터**로만 읽는다. **현재 상태의 SSOT는 `HANDOFF.md`.**
> 아래 본문의 진행률·마이그 번호는 작성 시점 값이라 낡았다. 실제로는:
> **B1-a(읽기 격리) 완료**(033~043) · **B1-b(쓰기 강제) 완료**(마이그112가 45개 테이블 sentinel DEFAULT 제거) ·
> **B2(가입 개방·초대·게스트) 완료**(113~116) · **B3(과금) 진행 중** — 나이스페이 계약 서명 완료, 결제 스키마·RPC·화면 배포됨(마이그139~140).
> 마이그 현재 **140**. 따라서 §0 "격리 0%·온보딩 동결"·§2-B1·§5 "대기"는 **해소됨**.
> 리스크 레지스터(B/H/M/L)는 우선순위 참고용으로 유지.
