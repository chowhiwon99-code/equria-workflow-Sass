...
>
> ⚠️ **현행화(2026-09-03)**: 이 문서는 **B3~B6 로드맵·H/M/L 리스크 레지스터**로만 읽는다. **현재 상태의 SSOT는 `HANDOFF.md`.**
> **B1(테넌트 격리)·B2(가입·초대·RBAC)는 완료**(마이그033~043·112~116) — 원래 여기 있던 §0·§1·§2-BLOCKER(B1~B6) 리스크 기록은
> `docs/archive/PRODUCTIZATION-B1B2-리스크.md`로 옮겼다(과거 진단, 지금은 사실 아님).
> **B3(과금)는 MVP 완료** — 나이스페이 정기결제 전 구간 배포·실결제 1건 증명(2026-08-20). 남은 건 카드사 승인심사(시한 09-17)와
> 마케팅 개시 조건뿐, 코드 작업은 끝남(상세 = `HANDOFF.md` §나이스페이 결제 연동).
> 마이그 현재 **143**. 아래 §2(H/M/L)·§3(B3~B6 로드맵)·§4·§5·부록은 지금도 유효한 참고 자료다.

---

## 0~1. B1·B2 — 완료됨

✅ **B1(테넌트 격리)·B2(가입·초대·RBAC) 모두 해소.** 착수 당시(2026-06-09)의 "두 번째 회사 받으면 데이터가 샌다"는 경고는
더 이상 사실이 아니다. 근거는 `HANDOFF.md` §🏢멀티테넌시. 원본 리스크 진단 전문은 `docs/archive/PRODUCTIZATION-B1B2-리스크.md` 참고.

---

## 2. 리스크 레지스터 (severity 순, B1·B2는 해소돼 제외 — 위 참고)
