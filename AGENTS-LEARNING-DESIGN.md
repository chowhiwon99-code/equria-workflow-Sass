# 에이전트 학습·기억 + 생성물 파이프라인 — 기획·기술 병목 분석 + v1 설계

> 상태: **v1(개인용 기억, §9)은 마이그099로 구현·배포 완료**(2026-07-16, 커밋 `a2d73be`). 그 외(생성물 파이프라인·RAG at scale·팀/조직 단위 기억 등 v2 이후)는 **여전히 기획/병목 분석 단계(구현 X)**. 2026-07-16(세션35) 대표 지시로 착수. 리서치 2갈래(개인 메모리 at scale · RAG/생성 파이프라인 at scale) 종합.
> 관계 문서: `AGENTS-MCP-STRATEGY.md`(에이전트 차별화·§4 온보딩 인터뷰) · `docs/archive/B1-DESIGN.md`(테넌트 격리, 구현 완료·과거 기록) · `HANDOFF.md`(현재 상태) · `PRODUCTIZATION.md`(로드맵).
