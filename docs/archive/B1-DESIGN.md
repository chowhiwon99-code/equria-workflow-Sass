...
> 상태(2026-06-14 현행화): **B1-a(읽기 격리, 마이그 033~043) = 구현·라이브 완료** → 아래 §3·§6의 033~041은 *as-built 기록*. **잔여 forward 계획 = B1-b(쓰기 강제: `WorkspaceProvider`·전 INSERT `workspace_id`·sentinel DEFAULT 제거)** = §4.8·§6 043~046·§7. ⚠️ 디스크 마이그 번호는 설계와 다름(예: 043=신규가입 멤버등록) — **번호보다 '작업 내용'으로 읽을 것.**
