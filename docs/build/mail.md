...
- ✅ **`MailCompose.tsx`로 1차 핵심 작성창 완료** — Tiptap 에디터, 받는사람·**참조·숨은참조 토글**, 서식 툴바(굵게·기울임·밑줄·**글자색·정렬**·글머리/번호 목록·링크), 첨부(3MB 합계 가드), AI 다듬기(`MailAiAssist.tsx`)까지 전부 코드로 존재·배선 확인.
  글자색·정렬은 2026-09-03에 추가(`@tiptap/extension-text-style`+`extension-text-align`) — ⚠️ 구글 계정 연결 필요 화면이라 **브라우저 실사용 확인은 아직 못 함**.
...
- B1-b 무관(개인 Gmail 연동).
- ~~서식 툴바 글자색·정렬~~ → ✅ 2026-09-03 추가. **남은 것**: 2차(예약 발송·서명·Drive 첨부·이모지·인라인 이미지·임시보관)는 착수 전.

## 검증 (E2E)
tsc0·lint27/0·build0 → 연결된 계정으로 새 메일: 받는사람·참조·숨은참조·서식(굵게/색/리스트)·첨부 → 전송 → 실제 수신 확인 → 답장 스레드 유지 확인.
