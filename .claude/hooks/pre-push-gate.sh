...
# 2) lint — 베이스라인 27 errors / 0 warnings. 회귀(에러>27 또는 경고>0)면 차단.
...
if [ -n "$counts" ]; then
  ...
  if [ "${errs:-0}" -gt 27 ] || [ "${warns:-0}" -gt 0 ]; then
    deny "🚫 배포 차단 — lint 회귀(에러 ${errs}/경고 ${warns}, 베이스라인 27/0). 신규 lint를 고치고 다시 push."
  fi
fi
