...
### I1b. eslint 부채 28건 — 배포는 안 막지만 코드품질 부채 (세션6 갱신)
- `next build`(=Vercel 빌드)는 **exit 0 통과**. **Next 16 Turbopack 빌드는 eslint를 게이트하지 않음** → 아래 에러들은 배포를 막지 않음.
- `pnpm lint`는 **28 errors + 0 warnings**(세션6: 신규 컴포넌트로 증가, 단 no-unused-expressions 경고 1건은 해소): `react-hooks/set-state-in-effect` ×26(전 컴포넌트 공통 `useEffect(()=>{load()},[load])` 패턴) · `react-hooks/refs` ×2(WorkflowCanvas·FloatingAgentChat).
