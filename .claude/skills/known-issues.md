...
- **I31. 크레딧 차감 계수가 원가와 1:1이라 만재 소진 시 적자(설계·HIGH)**: 1크레딧 = 원가 $0.01이라 Standard 포함 3,000 소진 = 원가 $30 > 매출 ₩29,000(≈$21). 현재는 breakage("대부분 다 안 씀")에 기대는 중이라 평균은 흑자지만 **헤비 유저 계정은 적자**. 계수를 $0.005로 바꾸면 마진 +29%가 되나 실질 한도가 절반 → **한도 설계(하이브리드 여부)와 함께** 결정해야 한다. 조정 지점 = `lib/credits.ts USD_PER_CREDIT` + 마이그133 트리거의 `×100`.
