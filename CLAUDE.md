...
> ⚠️ **작업이 끝나면 `.claude/skills/work-harness.md` §3-1 "작업 완료 보고 형식"(한 것 → 다음 할 일 A~Z 우선순위 + 이유 → 막힌 점 → "어느 것부터 진행할까요?")대로 보고한다. 대표 지정 고정 템플릿이며 생략 금지.**

---

## 1. 프로젝트 정체성

> ⚠️ **브랜드 규칙은 `HANDOFF.md` §합의된 정책이 최신(이 절을 대체).** 제품 **브랜드 = Complow(컴플로우)** 확정(도메인 `complow.kr`) · 코드 UI는 Complow로 교체 완료(내부 식별자 `equria:*`/`equria-*`/`equria.local`은 유지) · `K-뷰티`는 **첫 사내 고객 흔적** · 철학 = **회사별 커스터마이징**(도메인/브랜드는 고정값 아닌 슬롯/설정으로). **제품 목표 = 회사 AX(AI 전환) 도입·체계 구축 플랫폼** — 현금흐름 등 모듈을 업종 무관·회사별 커스터마이징·AI 보조로 제공. 아래 서술은 *첫 고객(우리 회사)* 맥락의 원본 기록.
...
## 6. DB 스키마 요약 (약 64개 테이블 · 마이그 001~156)
...
| 회의·아이디어·결정 | `meeting_notes`(+`transcript` 149·`search_text` 150·`project_id`/`calendar_event_id` 152) · `meeting_note_folders` · `meeting_categories`(070) · `meeting_action_items`(액션아이템·알림 트리거, 153) · `ideas`(아이디어 창고, 148) · `idea_graphs`(아이디어 지도 캐시, 154) · **`meeting_decisions`(결정 원장·번복 체인, 156)** · **`meeting_decision_scans`(재추출 방지 커서, 156)** — RPC `search_meeting_notes`·`search_decisions`(pg_trgm)·`link_decision_supersede` |
...
> 전체 SQL: `supabase/migrations/` (001~156, 135~137은 결번). 원격 적용·drift 없음. **세부 진행상황·최신 변경은 HANDOFF.md가 SSOT.**
