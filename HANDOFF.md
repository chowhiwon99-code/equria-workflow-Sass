# HANDOFF — 회사 AX(AI 전환) 도입 플랫폼 (사내 AI 워크스페이스 · B2B 전환 중 · 제품 브랜드명 미정)
...
> 최종 업데이트: **2026-07-02 (세션 25 — 손익 엑셀 전문가 계층표 + 계산 유형 만들기 쉽게).** ① **엑셀 재설계**(코드 `13ff398`): 상단 요약박스 + 구분(매출/비용/보유금)>그룹>항목 3단계·단계별 소계·총계 + **엑셀 네이티브 접기(+/−)**·색 섹션·라이브 수식(월별 제외). ② **계산 유형 UX**(코드 `70b924d`): 템플릿 검색·업종 카테고리 탭(28개 탐색) + 커스텀 **패턴 칩**(개수×단가/−차감/수수료%/부가세/마진/비율)로 스텝 조립 대체, 고급 스텝 유지. DB 변경 0. 이전(세션24): 유형 쉬운 이름·개수×단가 기본·타이핑 즉시 계산·표 접기(`9058b69`). 세션23 AI 코칭(`84b55ab`). **마이그 078~085 DB LIVE.** 상세=WORKLOG 세션15~25.
...
- **배포**: 프로덕션 `main` = **세션25 엑셀 계층표+계산유형 UX**(코드 `13ff398`·`70b924d`) · https://equria-workflow-sass.vercel.app (서울 icn1) · **롤백 후보**: `c872fb5`(세션25 전 = 세션24까지) · `e73de26`(계산 UX 전 = AI 코칭까지) · `9d353c3`(AI 코칭 전).
