...
> 최종 업데이트: **2026-07-11 (세션 32 — 오늘할일 시각알림 pg_cron(마이그 095) + 앰비언트 그라데이션 완화 + 모바일 픽스[iOS 채팅 자동확대·**사이드바 드로어 portal 탈출**(헤더 backdrop-filter가 fixed containing block 되던 회귀)·Modal body portal·viewport maximum-scale] · 프로덕션 **`0a44085`·롤백 `c954435`**·READY(`complow.kr`)).** 세션32 상세=WORKLOG. **⚠️ 실기기 육안 미검증(대표): 채팅 확대 해소·사이드바 드로어 불투명·그라데이션 톤·오늘할일 알림 도착.** 이전: **2026-07-10 (세션 31 — 프로젝트 체크리스트(세부 할 일) 배포 + 글래스모피즘 전면 리디자인 착수).** 프로젝트 체크리스트: 마이그 **094 project_tasks**(부모 `projects` 워크스페이스 격리를 **EXISTS로 상속** → 별도 workspace_id·센티넬 없음, 협업 CRUD) + `ProjectDetail` 체크리스트 섹션(추가·완료·삭제·진행률 바·Undo) + 공용 `lib/tasks.ts`(dueBadge 추출·TodayTasks 재사용). RLS 시뮬(anon 0/멤버 OK)·advisor 이상無·tsc0·lint30/0·build0. 프로덕션 **`c87e091`·롤백 `d44afc1`·Vercel READY(icn1)**. **🆕 글래스모피즘 전면 리디자인(P1~5) + 디테일 튜닝 + 비용·매출 개선 = 완료·배포**(계획=`~/.claude/plans/imperative-sprouting-crayon.md`, 상세=WORKLOG 세션31). **리디자인**: 토큰 우선(`globals.css` `--glass-*`·`--accent-dot`·`.glass`/`.glass-panel`·`.app-ambient` + `data-slot` 훅 + 신규 `shared/Surface` + `PageTransition` 페이드업, shadcn `ui/` 무편집) → 앰비언트 배경 + 프로스트 셸(사이드바·헤더 필 활성)/카드/드롭다운/모달. 캔버스·그리드·채팅·에디터·입력은 solid 유지. **디테일**: 글래스 강도 미세↑·카드 테두리로 칸 구분·호버 커짐 통일(z-index 침범방지). **비용·매출**: 요약 레이아웃(추세 그래프 축소 좌2/5 + 분류 우3/5)·추세 막대 그라데이션(초록/빨강)·'매출 채널'→'매출 분류'+분류 자유입력·사용분류 자동제안(업종무관). **전 단계 tsc0·lint30/0·build0.** 프로덕션 **`4e501d3`·롤백 `3093590`**(리디자인 전=`c87e091`, 세션30=`d44afc1`). **미검증(대표 육안): 3테마·Safari 블러 성능·모바일 드로어.** **이전: 세션 30 — 1차 기능묶음 + 후속: 프로젝트 중요도·생성시 참여인원·전자결재 안내·오늘할일 위젯 + 오늘할일 IME 중복픽스 + 대표의 구성원 부서 부여 / 마이그 091~093 원격 적용 / 프로덕션 `43552ec`·롤백 `9152ded` / 🌐 도메인 `complow.kr`+`www.complow.kr` LIVE).** 프로젝트: `projects.importance`(091, `lib/meetingMeta` 재사용)—생성폼 select·카드/상세 배지·인라인 변경(Undo) + 생성 모달 참여인원 다중선택→`project_members`(생성자=created_by라 pm_insert RLS 통과, Undo=cascade). 전자결재: **로직·RPC·스키마 무변경**, 헤더 접이식 "이렇게 써요" 3단계 배너(localStorage 유지)+빈상태 "새 기안" CTA+결재선 설명. 오늘 할 일: `personal_tasks`(092, 본인전용 RLS=`mcp_user_connections` 패턴)+`TodayTasks` 대시보드 위젯(추가·기한·완료체크·삭제·realtime·기한강조), **시각알림은 2차(pg_cron 스케줄러)로 분리**. eslint `.claude/worktrees/**` 무시(중첩 워크트리 lint 이중계상 방지). 검증 tsc0·lint30/0·build0·프리뷰+프로덕션 READY. 세션 시작 시 **전체 코드리뷰**(멀티에이전트): 보안/절대원칙·세션29 MCP 정합성 이상無(any0·키노출0·인증누락0·MCP user_id격리·OAuth PKCE·토큰암호화 정상). 마이너(미픽스): 개인커넥터 도구명 충돌 시 조용한 덮어쓰기·**GitHub 커넥터가 Copilot MCP 엔드포인트라 일반 PAT 권한 확인 필요**·MCP캐시 Map 키 프리픽스 없음. **후속(같은 세션 30):** ① 오늘할일 위젯 한글 IME Enter 이중발동→2개씩 추가 버그 픽스(`isComposing` 무시 + submitting ref 잠금, `3edb385`). ② 대표가 구성원 **부서** 부여(마이그 093 `set_member_department` 오너전용·063 패턴, 설정 "구성원 부서·직급" 행에 부서·직급 입력, 직원 자가입력 유지, `43552ec`). ③ **도메인 연결 완료**: `complow.kr`(A `@`→`216.198.79.1`)+`www.complow.kr`(CNAME `www`→`cname.vercel-dns.com`, 307→complow.kr) — 가비아 DNS + Vercel Domains(www redirect), SSL 자동. Vercel env `NEXT_PUBLIC_APP_URL`·`GOOGLE_OAUTH_REDIRECT_URI`=`https://complow.kr`로 변경+재배포(`9152ded`) + 구글 콘솔 OAuth JS원본/리디렉션 URI에 complow.kr 추가(대표 완료). **Vercel 식별자=memory `vercel-project`**. **다음 = 2차**(프로젝트 세분화 체크리스트 `project_tasks`·오늘할일 시각알림 pg_cron) · **백로그**: 회사 카드/금융 실연동(오픈뱅킹, "담당자만"=`064` 위임패턴 재사용)·카톡(카카오) 연결. **이전(세션 29 — 마이그 088~090, 세션30 `e2930ef`에 포함 배포됨):** MCP 대개편(대표 지적 "다 각자 계정 써야 하는거 아님?" 반영): GitHub·Supabase·Stripe를 회사 공용에서 **직원별 개인 연결**로 전환(`mcp_user_connections`, google_connections 선례 — 본인만 RLS, 관리자 게이트 없음) + **Notion·Linear·Sentry OAuth(DCR) 커넥터 신설**(사전등록 없이 "연결" 클릭만으로, `@ai-sdk/mcp` 내장 auth() 활용 — 설치 소스 직접 검증). 채팅·워크플로우가 요청자 본인의 개인 연결 도구를 에이전트 설정과 무관하게 항상 자동 병합. McpView "회사 전체 연결"/"내 연결" 섹션 분리 + bearer 연결 전용 간편 모달(기술 필드 숨김). Slack·Figma는 대표가 직접 개발자 포털 앱 등록 필요(coming_soon 유지) · Atlassian·Canva는 문서 상충으로 재확인 필요. **다음 = 실사용 검증**(대표가 Hugging Face→GitHub→Notion 순으로 직접 연결 테스트, OAuth 브라우저 왕복은 라이브 테스트 불가했음) → 배포. 이전(세션 29 전반 — 모바일 반응형 완료(배포 `41ccf58`) + MCP bearer 커넥터 개방(`de038b0`)). MCP: available 2→6종 — GitHub·Supabase·Stripe(bearer, "연결"→토큰 입력 모달 프리필)·Hugging Face(무인증 원클릭). 공식 엔드포인트 웹검색 확인. Linear·Sentry·Figma는 실제 OAuth라 coming_soon 유지. 등록 직후 자동 테스트. 모바일 3단계(채팅 위젯 클램프·메일 단일 pane·채팅 높이)까지 배포 완료. 전 페이지 감사A(캘린더 툴바·구성원 이메일·프로젝트 파일명·명함 헤더/이미지·세금계산서 표·재무 카드 스택 등 8건 픽스) · 현금흐름 폰=표 중심(캔버스 미마운트·내보내기/구조편집 PC 전용, 대표 확정) · 캘린더 폰=제목 첫 단어 · /mcp 폰=보기 전용 · Drive 탭 "Drive로 이동"(현재 폴더 딥링크) · **적대 리뷰(8앵글→10후보→7확정)**: 메일 100vh→dvh 누락·회의노트 문구·`useMediaQuery` 신설(캔버스/사이드바 배지 조건부)·FolderNameInput 추출. 세션29 전반: 사이드바 `hidden md:flex` + `MobileNav.tsx` 신규(md 미만 햄버거 → **z-[60]** 오버레이 드로어 — 앱 최초 z-50 초과 레이어, Sidebar 재사용·스크림 탭/링크 클릭/ESC 닫힘) · 모바일(<768px) `--app-pad` 0.75rem 변수 미디어쿼리(`--app-content-height` calc 자동 정합) · 쉘 `h-dvh`+calc `100dvh`(iOS 주소창 대응, 단독 롤백용 별도 커밋) · useUnreadDms 채널명 suffix(이중 마운트 방어). PC(≥md) 무변화. 대표 확정 3건 = dvh 포함·드로어가 FAB 위·패딩 0.75rem. **다음 = 스펙 작업2(단순 화면 감사)·작업3(메일/채팅 pane 전환·채팅 위젯 클램프)**(`docs/build/mobile-responsive.md`). 이전(세션 28 — MCP 디렉터리·토큰 암호화·비용 예산 + MCP 사용 흐름 명확화 + 워크플로우 에이전트·MCP 융합. 배포 `75d5944`, 롤백 `9af4751`·`4d6b215`). 세션28: /mcp Claude식 **커넥터 디렉터리**(검색·카테고리·정렬·추천·리치카드 13종) · **MCP bearer 토큰 DB 암호화 저장**(마이그 086 `mcp_servers.encrypted_token`, UI 입력·env 폴백·admin, crypto.ts 재사용) · **AI 비용 예산 한도**(마이그 087 `workspaces.monthly_budget_usd`, 월 예산+실행당 상한 $2, AI 라우트 9개 프리플라이트 429 차단·admin 예외·설정 UI) · **MCP 사용 흐름 명확화**(/mcp 3단계 안내 + "에이전트 만들기" 바로가기 → `/agents/new?mcp=` 프리필 + 채팅 🔧 도구 사용 칩) · **워크플로우 융합(B)**: 에이전트 노드가 자기 MCP 도구 사용(run 라우트, totalUsage 비용) + 캔버스 MCP 도구 노드 UI(픽커·인자 JSON 편집·플러그 아이콘) + MCP 클라이언트 런당 캐시·더블클로즈 픽스 · 대표(owner) role→admin 승격. (모바일 골격은 세션29에서 완료). 이전(세션 27 — Google Drive 탭 + Gmail 리치 작성/AI 다듬기 + 로딩 최적화·배포 `46cc1de`, 롤백 `676ddcd`). 세션27: `drive.readonly` Drive 탭(목록·폴더·다운로드) · Gmail 리치 작성창(참조/숨은참조·Tiptap 서식·첨부) + **AI 다듬기(격식체·친근체·간결·번역, AI 티 금지)** + 우측하단 도킹/모션 · Drive·Mail SWR 캐시+스켈레톤·**Gmail batch N+1 제거** · 회의노트 AI 이모지/줄표 제거 · 초안 랜딩 비공개+미연결 소셜 UI 제거. ⚠️ 프로덕션 Drive/Gmail 실사용 = 구글 OAuth Vercel env+콘솔 `drive.readonly`+prod redirect URI(대표). 이전(세션26): 제품 브랜드 **EQURIA/이큐리아 → Complow(컴플로우)** 확정. 코드 UI 브랜드 문자열 전체 교체(내부 식별자 `equria:*`/`equria-*`/`equria.local`은 유지) + 어시스턴트/에이전트 템플릿 K-뷰티 문구 업종무관 제네릭화. 도메인 `complow.kr`(국내 B2B) + 방어용 `.ai`/`.io`. 상표 9/42/35류 출원 예정. 사이드바 워드마크 Complow + Complow "C" 모노그램 favicon(`app/icon.svg`, 옛 favicon.ico 제거). **코드 배포 `859cec9`(브랜드)·`c5c33be`(로고·파비콘)**. 이전(세션25): 손익 엑셀 계층표(`13ff398`)·계산유형 UX(`70b924d`). **마이그 078~085 DB LIVE.** 상세=WORKLOG 세션15~26.
...
- **배포**: 프로덕션 `main` = **세션32 오늘할일 시각알림(마이그 095)+그라데이션 완화+모바일 픽스(채팅 확대·사이드바 드로어 portal·Modal portal)**(코드 **`0a44085`**, Vercel READY icn1·`complow.kr`) · Vercel 프로젝트 `complow-workflow-sass`(서울 icn1) · **롤백 후보**: **`c954435`**(사이드바 portal 전) · `fda0af6`(세션32 전=세션31 문서) · `4e501d3`(세션31 리디자인/비용매출) · `c87e091`(리디자인 전). *feat=main=origin 모두 `0a44085` 동기(클린).* ⚠️ **배포 교훈(재확인): main·feat를 같은 SHA로 연달아 push하면 Vercel 중복제거로 프로덕션 승격 스킵됨 → main 먼저 push하고 프로덕션 빌드 생성 확인 후 feat push(간격 필수).**
...
- **안정도**: `tsc` 0 · `pnpm lint` **30 errors/0 warnings**(전부 기존 `set-state-in-effect`·refs 부채, 신규 0이 베이스라인) · `any` 0 · 마이그 **001~095 적용·drift 없음** · **`next build` 성공** → **Vercel 빌드가 실제 게이트**.
...
6. **❓ 워크플로우 모바일 — 대표 답변 대기(세션32 미해결)** — 대표 "모바일에서 워크플로우 자연스럽게"를 *일반 모바일 흐름(채팅확대·사이드바)*으로 해석해 그쪽을 픽스함. **만약 워크플로우 기능 화면(n8n식 드래그 캔버스) 자체**를 폰에서 쓰기 불편하다는 뜻이었다면 → 폰에선 "보기/실행 중심"으로 단순화(드래그 캔버스는 데스크톱용)하는 별도 작업 필요. 다음 세션 시작 시 대표에게 어느 쪽인지 확인.
...
- **Supabase**: project `dutovtfdckhayyvhtuxu` (ap-northeast-2 서울) · 마이그 **001~095**. 🆕 세션32 마이그 095=`personal_task_reminders`(pg_cron 잡 `remind-due-personal-tasks` 매일 09:00 KST 활성).
