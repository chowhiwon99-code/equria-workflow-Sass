...
> **배포됨(LIVE, 프로덕션 `23cca30`, 마이그 096~098):** 기본 에이전트 제거+삭제UI · 예시 갤러리(백오피스 8종) · 카테고리/업무영역/직무 **자유입력** · 업계특화 문구 중립화 · 위젯 드래그 버그수정 · **지식파일 첨부**(AI-readable, 채팅 주입) · **개인 MCP 커넥터 바인딩**(빌더 '내 MCP 연결', 채팅·워크플로우 일관).
...
> 2. **P2.2 출력형식 재설계** — 9개 추상 multiselect → 업무언어 소수 단일선택 + 미니예시 + 직무기반 자동추천(`agentBuilder.ts` outputFormat).
