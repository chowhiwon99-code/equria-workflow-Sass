...
- **배포**: 프로덕션 `main` = **`fbca15b`** READY · https://equria-workflow-sass.vercel.app
  - **2026-06-11 세션 전체(23커밋)를 `feat→main` FF로 프로덕션 배포 완료.** 마이그 043~059는 이미 프로덕션 Supabase에 적용(프론트=DB 일치). **직전 `61592be`(비용추적)가 1클릭 롤백 후보.** `feat/toss-ui-refresh` = `main` 동일 시점.
