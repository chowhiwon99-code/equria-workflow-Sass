...
> 최종 업데이트: **2026-06-23 (세션 11 — 문서 정리 + A① 에이전트 매뉴얼(라이브 067) + 파일/회의노트 Finder식 폴더 UX·실제 썸네일 배포)**
...
- **배포**: 프로덕션 `main` = **`59317c8`+** READY · https://equria-workflow-sass.vercel.app (세션10 다수 배포 — 최신분은 위 로드맵 ✅ 참조)
...
- **🆕 macOS Finder식 폴더 UX + 썸네일(세션11 ✅ 배포·프론트만)**: 사이드바→**공용 `FolderGrid`**(중립 폴더 아이콘·더블클릭 진입·breadcrumb) · **다중선택+묶음 드래그+이동바** · 폴더 정렬·**날짜 그룹** · **실제 썸네일**(이미지 배치서명 + PDF=`pdfjs-dist` lazy·CDN 워커·실패 시 종류 아이콘). 파일·회의노트 통일, 명함 "직원별" 라벨. 구 `FolderSidebarItem` 삭제. **새 의존성 `pdfjs-dist`**(`next build` 통과 확인).
- **3차 ⬜ 남음(둘 다 大)**: ③재무 UI 재구성(탭/차트/기간필터, 통화합산 금지 유지) · ①그룹채팅(**단일 공유방** 권장 · 코어 RLS 수술 · 가장 신중)
