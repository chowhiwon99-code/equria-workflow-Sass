...
- **🔴 대표 액션(이것만 하면 바로 라이브 — 값 넣을 곳=설정→MCP 앱 크리덴셜):** ①⭐**구글**: Google Cloud Console(`console.cloud.google.com/apis/credentials`) → OAuth 동의화면 구성(스코프: `gmail.readonly gmail.compose calendar.events.readonly calendar.events.freebusy drive.readonly drive.file`) → '웹 애플리케이션' OAuth 클라이언트 생성 → **리디렉션 URI 3개** 등록(`https://complow.kr/api/mcp/oauth/google-gmail/callback`·`.../google-calendar/callback`·`.../google-drive/callback`) → 앱 **'게시(프로덕션)'** 전환(테스트=refresh 7일 만료) → 설정에 client_id/secret 입력. ②**Slack**: api.slack.com 앱 + 리디렉션 `.../oauth/slack/callback` → client_id/secret. ③**PayPal**: developer.paypal.com 앱 + 리디렉션 `.../oauth/paypal/callback` → client_id/secret.
