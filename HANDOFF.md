...
- ⚠️ **에이전트 채팅 39건 전부 `cache_write_tokens=0`·`cache_read_tokens=0`** — 캐시를 걸어둔 가장 비싼 경로에서 **캐시가 요청 단계에서 무산**되고 있다. 실제 히트 3건은 **컴피(assistant)** 쪽.
...
**실행 순서(권고)**: ①캐시 무산 원인 확정(진단) → ②캐시 복구+TTL 1h → ③내부 Haiku 라우팅 → ④`Notion clean` 정리(**대표 확인 필요**: 복잡 판단이면 Opus 유지+도구결과 누적만 차단 / 정리·분류면 Sonnet 전환 −40%) → ⑤효과 측정 후 필요시 무료 공정사용 3배→1배·지식파일 RAG·포함량 축소.
