...
- **배포**: 프로덕션 `main` = **세션34 에이전트 재설계 Phase 1(기본 8개 소프트삭제·마이그096 + 삭제UI + 빈섹션 숨김)**(코드 **`af87697`**, Vercel READY icn1·`complow.kr`) · Vercel 프로젝트 `complow-workflow-sass`(서울 icn1) · **롤백 후보**: **`ecadc69`**(Phase 1 전=세션33) · `34b9a61`(워크플로우 개선) · `c0aac03`(MCP·코칭·위젯). *feat=main=origin 모두 `af87697` 동기(클린, StepPicker 가독성 개선 1건은 워킹트리 미커밋 보류).* ⚠️ 마이그 096은 롤백 시 `update agents set is_active=true where created_by is null`로 기본 에이전트 복원 가능. ⚠️ **배포 교훈(재확인): main·feat를 같은 SHA로 연달아 push하면 Vercel 중복제거로 프로덕션 승격 스킵됨 → main 먼저 push하고 프로덕션 빌드 생성 확인 후 feat push(간격 필수).**
...
- **Supabase**: project `dutovtfdckhayyvhtuxu` (ap-northeast-2 서울) · 마이그 **001~096**. 🆕 세션34 마이그 096=`remove_default_agents`(기본 8개 소프트삭제, 복구가능). 세션32 095=`personal_task_reminders`(pg_cron 09:00 KST).
