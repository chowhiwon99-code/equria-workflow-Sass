...
> 최종 업데이트: **2026-07-03 (세션 26 — 브랜드 Complow 확정 + 코드/문서 교체·배포).** 제품 브랜드 **EQURIA/이큐리아 → Complow(컴플로우)** 확정. 코드 UI 브랜드 문자열 전체 교체(내부 식별자 `equria:*`/`equria-*`/`equria.local`은 유지) + 어시스턴트/에이전트 템플릿 K-뷰티 문구 업종무관 제네릭화. 도메인 `complow.kr`(국내 B2B) + 방어용 `.ai`/`.io`. 상표 9/42/35류 출원 예정. **코드 배포 `859cec9`**. 이전(세션25): 손익 엑셀 계층표(`13ff398`)·계산유형 UX(`70b924d`). **마이그 078~085 DB LIVE.** 상세=WORKLOG 세션15~26.
...
- **배포**: 프로덕션 `main` = **세션26 브랜드 Complow 교체**(코드 `859cec9`) · https://equria-workflow-sass.vercel.app (서울 icn1) · **롤백 후보**: `f1199cb`(세션26 전 = 세션25 엑셀/계산UX) · `c872fb5`(세션24까지) · `e73de26`(AI 코칭까지).
