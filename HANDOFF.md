...
- **배포**: 프로덕션 `main` = **세션33 MCP 커넥터 11종 추가 + AI 코칭 실장부 추세 인식 + 오늘할일 위젯 축소**(코드 **`c0aac03`**, Vercel READY icn1·`complow.kr`) · Vercel 프로젝트 `complow-workflow-sass`(서울 icn1) · **롤백 후보**: **`38242b9`**(세션33 전) · `0a44085`(사이드바 portal) · `4e501d3`(세션31 리디자인/비용매출). *feat=main=origin 모두 `c0aac03` 동기(클린).* ⚠️ **배포 교훈(재확인): main·feat를 같은 SHA로 연달아 push하면 Vercel 중복제거로 프로덕션 승격 스킵됨 → main 먼저 push하고 프로덕션 빌드 생성 확인 후 feat push(간격 필수).**
