...
1. **리디자인 최종 디테일 튜닝** — 대표가 **3테마(라이트/다크/공개화면 force-light)·모바일** 육안 확인 후: 글래스 강도·배경 톤(`.app-ambient`)·강조색(`--accent-dot`)·비용매출 레이아웃 비율(추세 2/5 vs 분류 3/5)·추세 그라데이션 진하기 등 미세조정. 전부 `globals.css` 토큰/소수 파일로 즉시 반영.
...
4. **오늘 할 일 시각 알림(2차·미착수)** — `personal_tasks.due_date` 기한 알림 pg_cron 스케줄러(세션30에서 1차와 분리 보류). 프로젝트 체크리스트(`project_tasks`)는 세션31에 완료.
