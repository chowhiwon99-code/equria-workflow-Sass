...
### 🆕 기능 로드맵 (세션10·2026-06-22 확정 · 대표 승인 · 진행 중)
> 8영역 멀티에이전트 코드조사 기반. 순서 = 안전 win→추가형→큰작업→AI고도화. 매 단계 예상이슈+검증(tsc/빌드/RLS시뮬) 후 보고.
- **1차**: ⑦위젯 둥둥모션 **[완료 `38455af` · 미배포]** → ②직급부여 → ④명함정리
- **2차**: ⑤근태 대표열람+위임 → ⑥회의노트 폴더
- **3차**: ③재무 UI 재구성 → ①그룹채팅(**단일 공유방** 권장 · 가장 신중)
- **4차**: ⑥회의노트 AI(웹서치·파일자동요약) + ＋AI초안(C 질문에이전트→A 위저드→B 템플릿워크플로우)
- **확정 결정**: ②직급 = `profiles.position` **자유입력 · 대표(owner)만 설정**(본인/타인 변경 불가). 역할부여 패턴(058/059) 복제. 그 외 세부(그룹방 단일공유·재무 탭 등)는 각 기능 착수 때 확정.
- **대표 결정 대기**: ⑥ 웹서치 = Anthropic 콘솔 활성+추가 과금 승인 필요.
