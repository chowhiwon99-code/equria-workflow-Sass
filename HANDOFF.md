...
- **안정도**: `tsc` 0 · `pnpm lint` **30 errors/0 warnings**(전부 기존 `set-state-in-effect`·refs 부채, 신규 0이 베이스라인) · `any` 0 · 마이그 **원격46↔디스크46 drift 없음** · 로컬 `next build`는 폰트 이슈로 미실행 → **Vercel 빌드가 실제 게이트**.
