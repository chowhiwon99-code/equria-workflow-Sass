...
> 현재 프로덕션 `58b6783` READY · main=feat=origin 동기·작업트리 clean·전부 push됨 · tsc0·lint29/0. **2트랙 대기(대표 승인=순서대로 진행):**

### 트랙1 — MCP 정적 클라이언트 인프라 → 구글·Slack·PayPal 연결 (분석 끝·코드 대기)
- **배경(검증됨 2026-07-24):** 구글은 **공식 원격 MCP 있음**(Gmail·Calendar·Drive, `developers.google.com/workspace/guides/configure-mcp-servers`)·Slack(`mcp.slack.com`)·PayPal 모두 **DCR 미지원** → 대표가 각 서비스에 OAuth 앱 등록해 받은 **client_id/secret**을 넣어야 함. 우리 OAuth(`src/lib/mcp/oauth.ts`)의 `clientInformation()`이 `mcp_oauth_clients`에서 client 정보를 읽으므로 **정적 client_id/secret을 주입하면 DCR을 건너뛰고 그대로 작동**(핵심 발견).
- **개발(내가 할 것):** ① 오너 설정에 "MCP 앱 크리덴셜"(커넥터별 client_id/secret) 저장 UI + admin으로 `mcp_oauth_clients` seed ② `lib/mcp.ts` 카탈로그에 **구글·Slack 커넥터 추가**(available·oauth) ③ **Zapier용 커스텀 URL 커넥터** 타입(사용자가 `mcp.zapier.com` 개인 URL 붙여넣기). ⚠️ 정적 client 경로에서 DCR 재등록 자가치유(마이그102) 로직이 정적값을 덮지 않게 가드.
- **⚠️ 대표 액션(병렬 선행 가능 — 값 주면 내가 배선):** ①⭐구글: Google Cloud Console → OAuth 동의화면 구성 → OAuth 클라이언트 ID/secret(Gmail·Cal·Drive 스코프) ②Slack: api.slack.com 내부앱 + `mcp:connect` 스코프+MCP 매니페스트 → client_id/secret ③PayPal: developer.paypal.com 앱 → client_id/secret.
- **보류/후순위:** Vercel=승인된 클라만(whitelist·불확실) · **Figma=사실상 막힘**(DCR 403·승인 클라 전용, 내부 SaaS 승인 대상 아님) · Exa=이미 활성(직원 각자 API키·대표 액션 0).
