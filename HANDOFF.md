...
### ⑧ DM 메시지 수정/삭제 + 읽음 카운터 정리 ⭐ 사용자 요청
- **삭제**: 본인 메시지에 호버 시 "삭제" 버튼. soft delete(`is_deleted=true` 컬럼 추가) 또는 hard delete + RLS로 본인 메시지만. UX는 "삭제된 메시지입니다" 또는 완전 제거.
- **수정**: 본인 메시지 호버 시 "수정". 텍스트 메시지만, 첨부 파일은 비활성. 수정 후 "수정됨" 표시 권장.
- **읽음 1 사라짐**: 카카오톡 스타일. 보낸이 입장에서 받는이가 안 읽으면 메시지 옆에 "1" 표시 → 읽으면 사라짐. 그룹은 미구현이라 1:1이라 1만 표시.
  - 현재 `direct_messages.is_read` 컬럼이 이미 있음 (안읽음 표시 기능 있음). 보낸이 입장에서 상대 is_read=false 메시지 카운팅해서 자기 메시지 옆에 "1" 표시 + Realtime 구독해서 상대가 읽으면 즉시 사라지게.
- **위젯 채팅(에이전트)도 동일?** 사용자에게 확인 필요. 보통 AI 채팅엔 안 적용.
- 핵심 파일: `direct_messages` 스키마, `components/chat/DirectChat.tsx`, RLS update 정책.

### ⑨ 캘린더 멀티데이 일정 — 드래그 생성 + 가로 막대 표시 ⭐ 사용자 요청
- **현재**: 멀티데이 이벤트는 시작일 셀에만 칩 표시. 중간 날짜는 비어 보임.
- **변경**: 시작~끝 날짜에 가로로 길게 이어지는 막대(span) UI. 색상은 6종 중 선택한 색.
- **드래그 생성**: 셀 mousedown → drag로 종료 셀 → mouseup → 자동 시작/끝 입력된 모달 띄움.
- **"색상도 흐리게"** 의미 확인 필요:
  - (a) 과거 일자(오늘 이전)는 흐리게 + 미래는 진하게?
  - (b) 모든 멀티데이 일정 전체를 흐린 톤?
  - (c) 진행 중인 일정만 흐리게?
  - 작업 전 사용자 의도 확인.
- 핵심 파일: `lib/calendar.ts`(날짜 그리드 유틸), `components/calendar/CalendarView.tsx`.
- 작업량: 드래그 인터랙션은 약간 까다로움. 1~2시간.
