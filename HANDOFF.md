...
## 🔜 다음 세션 바로 착수 (세션44[원가 레버 재조준·하이브리드 한도·크레딧 표현] 인계 · 2026-08-10 — 이 블록부터 읽기)

> **✅ 상태 = 로컬 커밋 3건(`4dd987d`·`aa69d68`·`3e66d01`) · 마이그 001~134**(134 원격 적용 완료·파일화 완료) · 각 tsc0·lint29/0(신규0)·build0.
> **🚧 아직 배포 안 됨** — 대표 승인 후 main-first 푸시 필요. 롤백 후보 = `cdb3fdc`(세션43 마지막 프로덕션).
...
**① 배포 + 대표 dogfood** — 위 3커밋은 아직 로컬. 배포 후 확인할 것:
- 무료 워크스페이스('테스트')로 전환 → 에이전트 만들기에서 **Opus가 잠기고 'Pro 이상' 배지**가 뜨는지
- 자동 실행(워크플로우) 소진 후 **채팅은 계속 되는지** ← ②의 핵심
- 사이드바 **'AI 사용량 N% 남음'** 표기 · 랜딩 요금표(행이 1개→2개로 늘어 레이아웃 확인 필요)

**② `agent_usage`에 라우트 식별 컬럼** — 지금 `agent_id`/`conversation_id`가 둘 다 null이라 **어시스턴트와 보조 라우트를 구분할 수 없다**. ⓪의 원가 분석도 토큰 크기로 추정할 수밖에 없었다. nullable `source` 한 컬럼이면 라우트별 원가가 보이고, ②의 interactive/automated 분류도 데이터로 검증된다. (추가형·저위험)
