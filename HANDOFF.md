...
> 최종 업데이트: **2026-07-01 (세션 24 — 현금흐름 계산 UX 대폭 단순화).** 사용자 피드백("유형 뭘 골라야 할지 모름·헷갈림·바로바로 계산") 반영: **유형 쉬운 이름**(직접 입력/개수 × 단가/채널 판매)·**칸 쉬운 이름**(개수/개당 가격/배송비)·**새 항목 기본=개수 × 단가**·**타이핑 즉시 계산**(onLive+astOf)·**캔버스 카드 유형 선택기**·**표 그룹 접기**·카드 계산칸 정렬. DB 변경 0. 코드 `9058b69`. 이전(세션23): **AI 코칭**(`/finance` 손익 스냅샷 Claude 분석, `84b55ab`). **마이그 078~085 DB LIVE.** 상세=WORKLOG 세션15~24.
...
- **배포**: 프로덕션 `main` = **세션24 계산 UX 단순화**(코드 `9058b69`) · https://equria-workflow-sass.vercel.app (서울 icn1) · **롤백 후보**: `e73de26`(계산 UX 전 = AI 코칭까지) · `9d353c3`(AI 코칭 전) · `95f65f1`(현금흐름 리뷰픽스 전).
