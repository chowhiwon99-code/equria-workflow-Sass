...
> 최종 업데이트: **2026-07-01 (세션 23 — 현금흐름 AI 코칭 배포).** `/finance` 손익 계산기에 **"AI 코칭"** 버튼 — 현재 손익 스냅샷(슬롯+통화별 요약)을 Claude가 분석해 **건강도·절감 제안·이상 신호** 구조화 카드로. **DDL 0·RLS 변경 0·읽기 전용**(generateObject 원샷·agent_usage 비용추적). **배포 `84b55ab` 프로덕션 LIVE.** 이전(세션15~21): 드래그 캔버스·그룹 컨테이너·회사 직접 편집 계산 칸(AST 수식 빌더)·함수 살아있는 엑셀/CSV·구분 색 자동. **마이그 078~085 DB LIVE.** 상세=WORKLOG 세션15~23.
...
- **배포**: 프로덕션 `main` = **`84b55ab`** (세션23: **현금흐름 AI 코칭**) · https://equria-workflow-sass.vercel.app (서울 icn1) · **롤백 후보**: `9d353c3`(AI 코칭 전 = dae6aae와 동일 코드·docs만) · `95f65f1`(현금흐름 리뷰픽스 전) · `3338917`(현금흐름 전체 전 = 세션13 그룹채팅).
