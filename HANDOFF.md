...
**④ 결제(PG) — 대표 액션이 임계경로**
- SNS 마케팅으로 가기로 결정 → **자동 결제 필요**(수동 청구는 아는 곳 전제였음). 결제 연동 코드는 **0줄**이고 PG 승인도 없다.
- PortOne에 **KCP 말고 다른 PG로 재신청 가능한지** 문의. 물어볼 것: B2B SaaS 월 구독 빌링 / 신설 법인·재무제표 없음 / 통신판매업 신고 완료 / **"불가하면 어떤 조건을 충족하면 되는지"**.
- 확인된 것: 토스페이먼츠 문서상 자동결제는 "리스크 검토 및 추가 계약 후" 가능하고 **정기 구독형은 정책 대상**. 업력 기준은 **어느 PG도 공개하지 않아 문의만이 답**.
