...
- **잔여(문서화·후속)**: **V5** 클라 활성워크스페이스↔서버 getUserWorkspaceId(첫 멤버십) 불일치 = 멀티멤버십 시 오귀속 → **워크스페이스 스위처와 함께 해결**(서버가 활성 ws를 헤더/바디로 받아 검증). **S2** 전역 profiles.role='admin' → B-5(회사별 권한) 전환 시 workspace_members.role로. **읽기 격리**: 현재 RLS가 "내 전(비게스트) 워크스페이스 UNION"을 보여줌 → 한 사람이 2개 회사 소속이면 목록이 섞임(현재 그런 유저 없음). 스위처가 활성 ws로 읽기 필터링해야 완결.
