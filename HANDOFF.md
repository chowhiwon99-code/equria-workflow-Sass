...
- **배포**: 프로덕션 `main` = **세션33 MCP 커넥터 11종 + AI 코칭 추세 인식 + 위젯 축소 + 워크플로우 사용성 개선(템플릿·통합 단계추가 피커·자동연결 명확화)**(코드 **`34b9a61`**, Vercel READY icn1·`complow.kr`) · Vercel 프로젝트 `complow-workflow-sass`(서울 icn1) · **롤백 후보**: **`8a8d902`**(워크플로우 개선 전) · `c0aac03`(MCP·코칭·위젯) · `38242b9`(세션33 전). *feat=main=origin 모두 `34b9a61` 동기(클린).* ⚠️ **배포 교훈(재확인): main·feat를 같은 SHA로 연달아 push하면 Vercel 중복제거로 프로덕션 승격 스킵됨 → main 먼저 push하고 프로덕션 빌드 생성 확인 후 feat push(간격 필수).**
