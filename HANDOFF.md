...
- 브랜딩 입력 완료(로고 `icon.png`·홈페이지·`/privacy`·`/terms`·승인도메인 `complow.kr`). **브랜드 검증 제출 여부 = 미확인.**
- ⚠️ **동의화면에 `dutovtfdckhayyvhtuxu.supabase.co`가 표시된다** — 브랜드 검증 통과 전엔 앱 이름 대신 콜백 호스트가 뜬다. 해결 = ① 검증 통과 대기(무료) ② **Supabase 커스텀 도메인**(공식 pricing 확인: **$10/도메인/월 + Pro $25/월**). **⚠️ 승인 도메인은 `complow.kr`인데 표시 도메인이 타사(`supabase.co`)라 브랜드 검증이 도메인 불일치로 반려될 수 있다** → 반려되면 ②가 사실상 필수. **지금은 결과 대기(선결제 금지).**
...
### 🔜 다음 세션 착수 순서
1. **신규가입 ON 직후 E2E 가입 검증** — 대표 타 구글 계정 → 워크스페이스 생성 확인 → 테스트 워크스페이스 정리
