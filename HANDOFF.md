...
- **배포**: 프로덕션 `main` = **`0030e9e`** (세션13: 회의 DB 뷰 + **AI 리서치 Part 2**(2a~2c) + **인터랙티브 지식 그래프**(꼬리물기·대화형) + 이미지크기·PDF) · https://equria-workflow-sass.vercel.app · **롤백 후보 `39dbb9d`**(카드 스레드 전) / `aa31ec0`(대화형 전) / `4d8c1e2`(꼬리물기 전) / `6314ae8`(그래프 전). 이전 배포분은 git 커밋·아래 로드맵 ✅ 참조.
