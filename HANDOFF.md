...
> - ✅ **① 만들기 진입 재설계 (배포 `d5ea4b9`)** — 예시 갤러리 **완전 제거** → **열린 "무슨 일 맡길지" 입력**이 메인 + 배경 **떠다니는 에이전트 아이콘 7개**(장식, `animate-float` 재사용). 흐름 = 입력→직무→**🆕 MCP 연결 스텝(직무 다음)**→나머지→생성. MCP 스텝 = 내가 연결한 커넥터 선택(`McpConnectorPicker`, 연결은 `/mcp` 선행·새탭). 미검증(대표 육안): 아이콘 톤·MCP 목록 자동갱신(새 연결 후 스텝 재진입 필요).
> - **② 학습/기억(최우선·스케일 필수)** — "계속 같으면 의미 없음". **앱레벨 메모리**(파인튜닝 아님): 세션 후 추출→다음 주입. v1=plain table 추출메모리+**편집/삭제 UI** → v2=pgvector top-k → v3 큐레이션(supersede·confidence·decay). **격리=RLS 강제**(테넌트 누출 방지·service_role 금지). 프롬프트 캐싱으로 비용↓(~$0.5~2/활성유저·월). 리서치 2갈래 완료(WORKLOG 세션35). **코드 前 설계 문서 먼저.** → ✅ **v1 배포(수동 기억 루프, `a2d73be`·롤백 `ebd30d2`, 2026-07-16)**: 마이그 099 `agent_memories`(개인용·격리검증 A=1/B=0) + 위젯 '기억 관리'(추가/목록/삭제·soft-delete) + 채팅 주입(활성 최근30). **방침=만들고 dogfood 후 고도화.** 다음=자동 추출·중복병합. 프로젝트공유=v1.5.**
...
> 5. **Phase 4 내보내기**(SKILL.md·AGENTS.md·시스템프롬프트 복사·MCP스니펫) · **Phase 5 워크플로우 사람검토 게이트**(재개형 실행+승인RPC, **선행: `save_file`(source="workflow")·`notify`(type="workflow") CHECK 위반 버그 수정** — 라이브 DB 확인 후).
