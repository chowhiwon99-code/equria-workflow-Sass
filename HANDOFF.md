...
> 📋 **전체 제품화 감사·리스크·B1~B6 로드맵·의사결정은 `PRODUCTIZATION.md`** / **B1 상세 설계는 `B1-DESIGN.md`**(2026-06-09) / **에이전트·MCP 차별화 전략·시한폭탄은 `AGENTS-MCP-STRATEGY.md`**(2026-06-10, 생각만/나중).
> 🧠 **에이전트/MCP 전략 핵심(AGENTS-MCP-STRATEGY.md)**: 철학=회사별 커스터마이징(브랜드 무관, 'EQURIA'·'K-뷰티'는 첫 사내고객 하드코딩 흔적·제품명 미정). 기본 에이전트 "멍청"은 프롬프트 콘텐츠 문제(짧음·예시0·절차0·온도 전부 0.7=seed 미설정→마이그 없이 수정 가능). **두 번째 회사 전 결정 필요**: ① 프리셋(기본 에이전트·MCP) 전략=복제/공용/하이브리드(권장 초기 복제) ② MCP 시크릿 전역환경변수→회사별 DB 암호화. **지금 안전·고가치(멀티테넌시 무관)**: 기본 에이전트 매뉴얼 재작성+온도값+회사지식 수집.
