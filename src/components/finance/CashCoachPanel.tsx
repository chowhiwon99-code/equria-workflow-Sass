...
import { Sparkles, X, RefreshCw, TrendingDown, AlertTriangle, Loader2 } from "lucide-react"
import { cn } from "@/lib/utils"
import type { CashAccount, CashCategory } from "@/types"
import type { CashSummary } from "@/lib/cashflowGraph"
import { buildCoachPayload } from "@/lib/cashCoach"
...
export function CashCoachPanel({
  ...
  const run = useCallback(async () => {
    ...
    setError(null)
    try {
      const payload = buildCoachPayload(slots, summaries, groups)
    ...
    <div className="rounded-lg border bg-card p-3">
      ...
      {health && (
        <div className="flex flex-col gap-3">
          ...
          {savings.length === 0 && anomalies.length === 0 && (
