...
export const WIZARD_FIELDS: WizardField[] = [
  ...
  {
    key: "successCriteria",
    label: "성공 기준 (이 결과가 '잘 됐다'는 걸 어떻게 아나요?)",
    type: "textarea",
    required: true,
    placeholder: "예: 담당자가 손대지 않고 그대로 발송 가능 / 숫자 근거가 다 들어감 / 3문장 이내로 요약",
    hint: "결과의 합격선을 적어주세요. 이게 명확할수록 AI 정확도가 크게 올라갑니다.",
    step: 2,
  },
  {
    key: "exampleScenario",
    label: "예시 입력 → 기대 결과 (강력 권장)",
    type: "textarea",
    required: false,
    placeholder: "입력 예시와 원하는 출력을 한 쌍 적어주면 품질이 크게 올라갑니다",
    hint: "비워두면 다음 단계에서 AI가 예시를 하나 만들어 확인받아요.",
    step: 2,
  },
]
...
4. "성공 정의"는 결과의 합격선을 **측정 가능하게** 씁니다(예: "담당자가 수정 없이 발송 가능", "모든 수치에 근거 명시"). 사용자가 성공 기준을 줬으면 그대로 반영하고, 없으면 목적·출력형식에서 합리적 합격선을 도출합니다. 이 기준은 "작업 절차"의 자기점검 단계에도 연결합니다.
5. 가드레일을 기본 포함합니다(업종 무관): 근거 없는 단정·과장 금지, 사실은 사실로/추정은 추정으로 구분, 법적·규제 리스크 소지 표현 주의, 확신이 없으면 지어내지 말고 사용자에게 되묻기.
6. 에이전트 이름이 비어 있으면 목적에 맞는 간결한 이름을 직접 지어 첫 줄에 사용합니다.
7. 정보가 부족하면 임의로 진행하지 말고 먼저 질문하라는 규칙을 "작업 절차" 또는 "엣지 케이스"에 포함합니다.
8. 입력 뒤에 "사용자 추가 답변(인터뷰)" 블록이 있으면, 그 내용을 **가장 우선하는 근거**로 삼아 관련 섹션(특히 예시·성공 정의·제약·컨텍스트)을 구체화합니다.`
